...
def load_cifar10_data(batch_size: int = 128, num_samples: Optional[int] = None) -> Tuple[DataLoader, DataLoader]:
...
def evaluate_accuracy(models, dataloader: DataLoader, device: torch.device, is_batch: bool = False) -> List[float]:
...
def verify_equivalence(sequential_accuracies: List[float], batch_accuracies: List[float], 
...
def train_sequential(models, trainloader: DataLoader, num_epochs: int, 
...
def train_modelbatch(models, trainloader: DataLoader, num_epochs: int,
...
def run_benchmark(num_models: int = 16, num_epochs: int = 5, batch_size: int = 128, 
    ...
    # Load data
    trainloader, testloader = load_cifar10_data(batch_size=batch_size, num_samples=num_samples)
    print(f"Training samples: {len(trainloader.dataset)}, Test samples: {len(testloader.dataset)}")  # type: ignore
    ...
    sequential_accuracies = evaluate_accuracy(sequential_models, testloader, device, is_batch=False)
    
    equivalent = verify_equivalence(sequential_accuracies, batch_accuracies, learning_rates, dropout_rates)
    
    # Memory usage
    if torch.cuda.is_available():
        memory_used = torch.cuda.max_memory_allocated() / 1e9
        memory_total = torch.cuda.get_device_properties(0).total_memory / 1e9
        print(f"GPU Memory: {memory_used:.2f}GB / {memory_total:.1f}GB ({memory_used/memory_total*100:.1f}%)")
